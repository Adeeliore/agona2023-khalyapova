...
function logPerson(person: Person, index: number) {
    ...
    let dateDetails = dateWizard.dateDetails(person.registered);
    let registeredAt = `${dateDetails.date}.${dateDetails.month}.${dateDetails.year} ${dateDetails.hours}:${dateDetails.minutes}`;
